...
// fiche : http://data.grandlyon.com/environnement/corbeilles-de-rue-sur-le-territoire-du-grand-lyon/

$sourceDonnees = 'https://download.data.grandlyon.com/wfs/grandlyon?SERVICE=WFS&VERSION=2.0.0&outputformat=GEOJSON&request=GetFeature&typename=gin_nettoiement.gincorbeille&SRSNAME=urn:ogc:def:crs:EPSG::4326';
...
foreach($Donnees["features"] as $s) {
	...
	pg_query($dbconn, "INSERT INTO $BD_table
		(type, commune, voie, numerodansvoie, gestionnaire, observation, identifiant, gid, geom)
		VALUES ('$type', '$commune', '$voie', '$numerodansvoie', '$gestionnaire', '$observation', '$identifiant', '$gid', ST_SetSRID(ST_MakePoint($longitude , $latitude), 4326) )")
