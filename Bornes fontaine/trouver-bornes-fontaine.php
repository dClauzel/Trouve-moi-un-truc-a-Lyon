...
*/
$tmpfile = uniqid(sys_get_temp_dir()."/BORNE-FONTAINE_");
...
$i=0;
while($NbResultats<3) {
	echo "document.getElementById('resultat$NbResultats').innerHTML = '". GL_BorneFontaine_Affiche($Donnees["features"][$i]). "';\n";
	echo "var marker = new google.maps.Marker({
		...
		map: map,
		title: '" .addslashes($Donnees["features"][$i]["properties"]["nom"]). "'
		});\n";
	$NbResultats++;
	$i++;
}
